...
async function handleStreamingRequest(event, context, timeoutDuration = 45000) {
  try {
    ...
    const { message, sessionId, uploadedFiles, currentUser } = requestBody;
...
exports.handler = async (event, context) => {
  ...
  try {
    const requestBody = JSON.parse(event.body);
    streaming = requestBody.streaming === true;
  } catch (parseError) {
  ...
  try {
    ...
    console.log('Extracted message:', message, 'SessionId:', sessionId);
    ...
    // For long messages, go straight to background processing
    if (useBackgroundProcessing) {
      console.log('📊 LONG MESSAGE: Going straight to background processing');
      
      const taskId = `task-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      console.log('🏃‍♂️ STARTING BACKGROUND TASK:', taskId);
      
      // Call the langflow-background function directly (internal call)
      console.log('🚀 BACKGROUND: Triggering langflow-background function');
      
      // Call background function with proper URL
      const backgroundUrl = 'https://ks-chat-test.netlify.app/.netlify/functions/langflow-background-task';
      console.log('🚀 BACKGROUND: Calling background function at:', backgroundUrl);
      ...
      fetch(backgroundUrl, {
        ...
        console.log('📞 BACKGROUND: Call response status:', response.status);
        if (!response.ok) {
          console.error('❌ BACKGROUND: Call failed with status:', response.status);
        } else {
          console.log('✅ BACKGROUND: Call successful');
        }
      }).catch(err => console.error('❌ BACKGROUND: Call error:', err));
      ...
      return {
        ...
        body: JSON.stringify({
          message: 'Je vraag wordt verwerkt op de achtergrond. Dit kan even duren...',
          taskId: taskId,
    ...
    // If short message timed out, also use background processing  
    if (useBackground) {
      console.log('⏰ SHORT MESSAGE TIMEOUT: Switching to background processing');
      
      const taskId = `task-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      console.log('🏃‍♂️ STARTING BACKGROUND TASK (timeout fallback):', taskId);
